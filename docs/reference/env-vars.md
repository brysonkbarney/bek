...
| Slack Web API posting        | `SLACK_BOT_TOKEN`                                                                                       | Optional self-hosted/local posting. Hosted installs still need credential-vault storage for exchanged bot tokens.                                    |
...
| `SLACK_BOT_TOKEN`                                                                                                                   | Active for Slack posting    | Consumed by the API for outbound Slack Web API posting. OAuth exchange can return a bot token, but Bek redacts the callback response and does not persist it yet. |
