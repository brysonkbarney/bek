...
| Slack OAuth install/callback | `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, `SLACK_STATE_SECRET`, `SLACK_REDIRECT_URI`, `BEK_CREDENTIAL_MASTER_KEY` | Redirect/state validation works; code exchange stores bot tokens in the local encrypted vault. Hosted installs still need managed KMS/broker custody. |
